...
from final.entidade.treinador import Treinador
from final.view.tela_pokemon import TelaPokemon
from final.controladores.controlador_item import ControleItem
from final.telas_gui.tela_admin import TelaAdmin
...
class ControleTreinador:
    ...
    def abre_tela(self):
        evento,valores=self.__tela_admin.abrir()
        if evento=='voltar':
            self.__tela_admin.fechar()
            self.__controlador_sistema.entrar()
        if valores['incluir']==True:
            self.__tela_admin.fechar()
            self.adicionar_treinador()